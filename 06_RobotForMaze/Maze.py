...
class Maze(object):
    """
    is_hit_wall(self, location, direction): 判断是否撞墙
    set_reward(self, reward=None): 设置奖励
    move_robot(self, direction): 移动机器人
    sense_robot(self): 获取机器人位置
    reset_robot(self): 重置机器人位置`
    draw_robot(self): 绘制机器人
    generate_maze(self, maze_size): 生成迷宫
    draw_maze(self): 绘制迷宫
    is_edge(self, wall, shape): 判断是否为边缘
    can_move_actions(self, position): 获取机器人能够合法移动的方向
    
    迷宫对象有几个主要的属性：
        maze_data：每个单元格的墙壁条件都被编码为一个4位数，如果有墙壁，位值取0，如果没有墙壁，位值取1。1s寄存器对应一个方格的顶边，2s寄存器对应右边，4s寄存器对应底边，8s寄存器对应左边。
    """
